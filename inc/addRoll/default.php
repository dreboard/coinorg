...
function newCoinSelects($coinType, $i){
   ...
   $html = ' <tr><td width="10%">Coin '.$i.'</td><td width="38%">';
   ...
   while($row = mysql_fetch_array($sql))
		{
		  ...
		  $strike = $coin->getCoinStrike();
		  $html .= '<option value="'.$coinID.'">'.$coin->getCoinName().'</option>';	
		}	
		$html .= '</select></td>';
		$html .= '<td width="14%">Grade </td><td width="38%"><select name="coinID['.$i.'][coinGrade]">';
		switch($strike){
			case 'Business':
			$html .= '<option value="B0">B-0</option>
...
for ($i=1; $i<=$CoinTypes->getRollCount(); $i++)
  {
  echo newCoinSelects($coinType, $i);
  }
?>
</table>
