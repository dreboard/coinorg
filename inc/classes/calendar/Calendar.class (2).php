...
function draw_calendar($month, $year, $loc){
	...
	for($list_day = 1; $list_day <= $days_in_month; $list_day++):
		$calendar.= '<td valign="top">';
			/* add in the day number  and  QUERY THE DATABASE**/
			
		  switch ($loc){
			  case "front":
				  $calendar.= '<div class="daynumber">'.$list_day.'</div><div class="dayNumber" id="dayNumber'.$list_day.'">'.$Event->multiEventFrontDisplay($eventStartDate=$year.'-'. str_pad($month, 2, '0', STR_PAD_LEFT).'-'. str_pad($list_day, 2, '0', STR_PAD_LEFT));
				  break;
			  case "back":
