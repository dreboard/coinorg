...
class User
{
    ...
    public function getPCGSLink()
    {
        return strip_tags($this->pcgs);
    }

    public function getNGCLink()
    {
        return strip_tags($this->ngc);
    }

    public function getAnaNum()
    {
    ...
    {
        return strip_tags($this->viewLevel);
    }
...
//User Awards
    ...
    */
    ...
    public function getAnswerAvatar($userID)
    {
        $stmt = $this->db->dbhc->prepare("SELECT COUNT(DISTINCT question_id) FROM forum_answer WHERE userID = :userID");
    ...
    {
        $stmt = $this->db->dbhc->prepare("SELECT COUNT(DISTINCT coinID) FROM collection WHERE userID = :userID");
        $stmt->bindParam(':userID', $userID, PDO::PARAM_INT);
        $stmt->execute();
    ...
    {
        $stmt = $this->db->dbhc->prepare("SELECT COALESCE(sum(purchasePrice), 0.00) FROM collection WHERE userID = :userID");
        $stmt->bindParam(':userID', $userID, PDO::PARAM_INT);
        $stmt->execute();
