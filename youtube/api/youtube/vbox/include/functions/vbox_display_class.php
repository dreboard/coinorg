...
class Vbox_display_class {
	...
	function displayVideosListFacebookLikeByVideosDatas($videosData,$criteria) {
		...
		for($i=0;$i<count($videosData['videos']);$i++) {
			...
			echo '<div class="itemBox" id="'.$videoid.'" type="'.$videoType.'" style="padding-top:10px; overflow:hidden">';
			
				echo '<div class="videoPlayBox thumbnailBox" style="float:left;">';
					echo '<a href="'.$url.'" class="displayVideoPlayer">';
					echo '<img src="'.$videoThumbnail.'" style="width:80px; height:55px; padding-right:30px;" border=0>';
					echo '<span class="play"></span>';
					echo '</a>';
				echo '</div>';
				
				echo '<p><a class="displayVideoPlayer black" href="'.$url.'">'.$title.'</a><br>';
				echo '<font color="#736F6E"><small>'.$gf1->formatDuration($duration);
				echo ' - Views: '.number_format($viewCount).'</small></font></p>';
				
				echo '<div style="display:none" class="video_details">';
				if($description!='') echo '<div><b>Description:</b> '.$description.'</div>';
				if($category!='') echo '<div><b>Category:</b> <a href="./index2.php?t=5&cat='.$category.'">'.$category.'</a></div>';
				if($tags!='') echo '<div><b>Tags: </b>'.$tags.'</div>';
				echo '</div>';
