...
if (isset($_POST['privacyBtn'])) {
	...
	if($CoinClub->privacyUpdater($userID, $password, $viewLevel)){
...
if (isset($_POST['emailBtn'])) {
	...
	if($CoinClub->emailUpdater($userID,$mailPassword, $newEmail)){
...
if (isset($_POST['passwordFormBtn'])) {
	...
	if($CoinClub->passwordUpdater($userID,$oldPassword, $newPassword)){
       ...
       $login_check = mysql_num_rows($loginQuery);
		   ...
		   if(!empty($_FILES['file']['tmp_name'])){
			   ...
			   if(($_FILES["file"]["size"] > 3000000)){
				$_SESSION['pageMsg'] = 'Image is too large';
				}else{
				...
				$Upload->UploadFile();
				$imageURL = $userID."/" . $User->getUserName().str_replace(' ', '_', $_FILES['file']['name']);
...
$('#viewLevel').click(function(){
	if ($('#viewLevel option:selected').text() == "Private"){
    $("#privateMsg").show().addClass("errorTxt");
	}else if ($('#viewLevel option:selected').text() == "Public"){
	$("#privateMsg").hide();
	}
}); 
...
<h2 id="pageHdr"><img src="<?php echo $User->getImageURL() ?>" alt="" align="left" class="imageURL" />&nbsp; Edit Club Account Details<br />
...
<h2>Change Privacy Level</h2>
		...
		<tr>
		<td colspan="4" align="left">Please enter your password to change your club privacy level, (links only viewable to members).</td>
		</tr>
		<tr>
		  <td align="right" valign="top"><label for="privatePassword">Password</label></td>
		  <td width="22%"><input name="privatePassword" id="privatePassword" type="password" /></td>
		  <td width="39%">&nbsp;</td>
		  <td id="cancelMsg2">&nbsp;</td>
		  </tr>
		<tr>
		<td width="20%" align="right" valign="top"><label for="viewLevel" id="viewLevelLbl">Privacy Level:</label></td>
		<td colspan="2"><select id="viewLevel" name="viewLevel">
        <option value="<?php echo $User->getPrivacyLevel() ?>" selected="selected"><?php echo $User->getPrivacyLevel() ?></option>
            <option value="private">Private</option>
            <option value="public">Public</option>
            </select> 
		<span id="privateMsg">Prospective Members will not see club information</span></td>
		<td width="19%" id="cancelMsg">&nbsp;</td>
		</tr>      
		<tr>
		<td>&nbsp;</td>
		<td colspan="3"><label>
...
<h2>Cancel Club Account</h2>
		<form action="" name="cancelForm" id="cancelForm" method="post">
		<table width="100%" border="0" cellspacing="0" cellpadding="6" id="accountFormTbl">
		<tr>
		<td colspan="3" align="left">Please enter your password to cancel your club account with <span id="TopPanelDF"><span id="gh-ug"><strong>My Coin Organizer</strong></span></span>, and to delete all account files.</td>
		</tr>
		<tr>
