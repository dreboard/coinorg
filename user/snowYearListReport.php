...
<title><?php echo str_replace('_', ' ', $_GET["coinYear"]); ?> <?php echo str_replace('_', ' ', $_GET["coinType"]); ?>  Snow Report</title>
...
$("#snowSwitchForm").submit(function() {
      if ($("#snowSwitch").val() == "") {
		$("#snowSwitch").addClass("errorInput");
        return false;
      }else {
...
<h2><img src="../img/<?php echo $_GET["coinType"]; ?>.jpg" alt="Obverse and reverse" width="50" height="50" align="absmiddle" title="<?php echo $coinType; ?>" /> <?php echo str_replace('_', ' ', $_GET["coinYear"]); ?> <?php echo str_replace('_', ' ', $_GET["coinType"]); ?>  Snow Report</h2>
...
<hr />
  ...
  <table width="100%" border="0">
    <tr>
      <td width="32%"><h3><?php echo $coinYear; ?> Snow List (Album View) </h3></td>
      <td width="35%"><select id="select6" name="select7" onchange="window.open(this.options[this.selectedIndex].value,'_top')">
        ...
        <option value="snowYearReport.php?coinYear=<?php echo $coinYear; ?>&amp;coinType=<?php echo $_GET["coinType"]; ?>">Album View</option>
      </select></td>
      <td width="33%" align="right">
<form action="snowYearReport.php" method="get" class="compactForm" id="snowSwitchForm">
<select class="yearSwitch" name="coinYear" id="snowSwitch">
...
<input type="submit" value="Load Year" class="yearSwitch" id="snowSwitchBtn" />
</form>      
      </td>
    </tr>
  </table>
...
$countAll = mysql_query("SELECT * FROM collection WHERE coinType = '".str_replace('_', ' ', $coinType)."' AND coinYear = '$coinYear' AND userID = '$userID' AND snow != 'None' ORDER BY coinYear ASC") or die(mysql_error());

if(mysql_num_rows($countAll) == 0){
	  echo '
		  <tr>
		  <td width="57%"><a href="addCoinType.php?coinType='.$_GET["coinType"].'"><strong>No '.$coinType.'\'s assigned Snow</strong></a></td>
  ...
  echo '
    <tr align="center">
    <td align="left">'.$coinIcon.'<a href="viewCoinDetail.php?ID='.$Encryption->encode(intval($row['collectionID'])).'">'.substr($coin->getCoinName(), 0, 50).' '. $collection->getCoinGrade() .$grader.' <strong>'.$collection->getSnow().'</strong></a></td>
