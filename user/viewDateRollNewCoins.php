...
<h1><img src="../img/<?php echo $coinLink ?>.jpg" width="80" height="80" align="absmiddle" /> Add Coins to <?php echo $rollNickname ?> Roll</h1>
 <p><a href="viewTypeRollDetail.php?ID=<?php echo $Encryption->encode($collectrollsID) ?>" class="brownLinkBold">Roll Main View</a> | <a href="RollTypes.php?rollType=<?php echo str_replace(' ', '_', $rollType); ?>"><?php echo $rollType; ?></a></p>
 <hr />
...
<form action="" method="post" enctype="multipart/form-data" name="addCentForm" id="addCentForm">
 ...
 <p>Or. You can add more detail to each coin entering it one at a time at <a href="addCoinType.php?coinType=<?php echo str_replace(' ', '_', $collectionRolls->getCoinType()); ?>" class="brownLinkBold">Add A Coin</a>.  Slabbed coins will not be available for rolls. To change the year range you can <a href="editRollDateRange.php?ID=<?php echo $Encryption->encode($collectrollsID) ?>" class="brownLinkBold">Edit This Roll Details</a>.</p> 
...
<table width="100%" border="0">
  ...
  for ($i=1; $i<=$optionLimit; $i++) {	
	$sql = mysql_query("SELECT * FROM coins WHERE coinType = '".$collectionRolls->getCoinType()."' AND coinYear BETWEEN '".$collectionRolls->getStartYear()."' AND '".$collectionRolls->getEndYear()."' AND byMint = '1'  ORDER BY coinYear ASC") or die(mysql_error()); 
