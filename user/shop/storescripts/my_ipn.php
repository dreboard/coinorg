...
if (strpos($curl_result, "VERIFIED") !== false) {
	...
	mail("admin@alikaimartialarts.com", "IPN interaction not verified", "$req", "From: admin@alikaimartialarts.com" );
	exit();
}
...
if ($receiver_email != "admin@alikaimartialarts.com") {
	$message = "Investigate why and how receiver email is wrong. Email = " . $_POST['receiver_email'] . "\n\n\n$req";
    mail("admin@alikaimartialarts.com", "Receiver Email is incorrect", $message, "From: store@alikaimartialarts.com" );
    exit(); // exit script
}
// Check number 2 ------------------------------------------------------------------------------------------------------------
...
// Connect to database ------------------------------------------------------------------------------------------------------
require_once '../../../MyOnlineStore - Copy/storescripts/connect_to_mysql.php';
// Check number 3 ------------------------------------------------------------------------------------------------------------
...
if ($numRows > 0) {
    ...
    mail("admin@alikaimartialarts.com", "Duplicate txn_id in the IPN system", $message, "From: store@alikaimartialarts.com" );
    exit(); // exit script
} 
// Check number 4 ------------------------------------------------------------------------------------------------------------
...
foreach ($id_str_array as $key => $value) {
	...
	$sql = mysql_query("SELECT price FROM products WHERE id='$id' LIMIT 1");
...
if ($fullAmount != $grossAmount) {
        ...
        mail("admin@alikaimartialarts.com", "Price Jack or Bad Programming", $message, "From: store@alikaimartialarts.com" );
        exit(); // exit script
} 
...
mail("admin@alikaimartialarts.com", "Store IPN RESULT YAY MONEY!", $req, "store@alikaimartialarts.com");
?>