...
echo '<td width="25%"><a href="clubAlbumView.php?ID='.$Encryption->encode($albumID).'"><img title="'.$Album->getAlbumName().'" class="thumb" src="'.$Album->albumFirstPic($albumID).'" /></a><br />
<strong>Images:</strong> '.$Album->getAlbumImgCount($albumID).' <br /><a href="clubAlbumView.php?album='.$Encryption->encode($albumID).'&ID='.$Encryption->encode($coinClubID).'">'.$Album->getAlbumName().'</a></td>'; 
$i = $i + 1; //add 1 to $i
