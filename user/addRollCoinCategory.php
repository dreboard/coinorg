...
$CoinTypes->getCoinByType(str_replace('_',' ',  $_GET["coinType"]));
}
...
mysql_query("INSERT INTO collectrolls(rollNickname, coinID, coinVersion, rollType, coinType, coinCategory, rollHolder, purchaseDate, purchaseFrom, auctionNumber, additional, purchasePrice, ebaySellerID, shopName, shopUrl, coinNote, coinGrade, enterDate, userID, coinYear, mintMark, faceVal, coincollectID, containerID) VALUES ('$rollNickname', '$coinID', '".$coin->getCoinVersion()."', 'Same Coin', '".$coin->getCoinType()."', '".$coin->getCoinCategory()."', '$rollHolder', '$purchaseDate', '$purchaseFrom', '$auctionNumber', '$additional', '$purchasePrice', '$ebaySellerID', '$shopName', '$shopUrl', '$coinNote', '$coinGrade', '$theDate', '$userID', '".$coin->getCoinYear()."', '".$coin->getMintMark()."', '".$CoinTypes->getRollVal()."', '".intval($_POST["coincollectID"])."', '".intval($_POST["containerID"])."')") or die(mysql_error());
...
if($_POST['addCoins'] == 'myCoins'){		
for ($i = 1; $i <= $CoinTypes->getRollCount(); $i++)
{ 
$collectionRolls->enterNewRollOpenCoins($collectrollsID, $coinID, $userID, $purchaseFrom, $auctionNumber, $additional, $purchasePrice, $ebaySellerID, $shopName, $shopUrl, $showName, $showCity,  $purchaseDate);
}
}
...
$(".detailDiv2, .addRollMsg").hide();

$("#purchaseFrom").change(function() {
	$(".detailDiv2").hide();
	$('#' + $(this).val()).show();
});	
$("#addCoins").change(function() {
	$(".addRollMsg").hide();
	$('#' + $(this).val()).show();
});	
...
<table width="100%" border="0">
  <tr>
    ...
    <img src="../img/<?php echo strip_tags($_GET["coinType"]); ?>.jpg" width="80" height="80" align="absmiddle" />
    </td>
    <td width="91%"><h2> Add  <?php echo str_replace('_',' ',  strip_tags($_GET["coinType"])); ?> Roll</h2></td>
  </tr>
  <tr>
    <td><strong>Roll Type:</strong> <a href="addRollSameCoinCollection.php">Same Year &amp; Mint</a></td>
  </tr>
</table>
...
<?php 
$collectionQuery = mysql_query("SELECT * FROM collectrolls WHERE rollType = 'Same Coin' AND coinType = '".str_replace('_', ' ', $_GET["coinType"])."' AND userID = '$userID' ORDER BY collectrollsID DESC LIMIT 5") or die(mysql_error());
...
if(isset($_SESSION['errorMsg'])) {
 ...
 ?>&nbsp;</td>
  ...
  <tr>
    <td width="211" class="darker">Year/Mint</td>
  ...
  {
	 echo '<option value="'.intval($row['coinID']).'">'.strip_tags($row['coinName']).'</option>';	
  }
 ?>
    </select></td>
  </tr>
  <tr>
    ...
    <td><select name="addCoins" id="addCoins">
      ...
      <option value="newCoins">Yes, Add All <?php echo $CoinTypes->getRollCount(); ?> Coins</option>
      <option value="myCoins">Add All Open Coins</option>
    </select> 
     <span id="newCoins" class="addRollMsg">(All new coins will be added to roll)</span>
     <span id="myCoins" class="addRollMsg">(All existing coins will be added to roll)</span></td>
  </tr>
    </table>
...
if(isset($_POST["coincollectID"])){echo '<option value="'.$_POST["coincollectID"].'" selected="selected">'.$_POST["coincollectID"].'</option>';
		...
		echo  '<option value="0" selected="selected">No</option>'.
		$Container->getOpenCategoryOptions($userID, str_replace('_',' ',  strip_tags($_GET["coinCategory"])));
 }?>
    </select></td>
  </tr>  
  </table> 
