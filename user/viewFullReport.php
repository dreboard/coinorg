...
if(isset($_GET["coinType"])){
	...
	switch(strip_tags(str_replace('_', ' ', $_GET["coinType"]))){
		...
		break;
		case 'Roosevelt Dime': 
		$fullType = 'Full Band/Torch';
		$fullAtt ="(fullAtt = 'FT' OR fullAtt = 'FB')";
		break;		
		case 'Franklin Half Dollar': 
		$fullType = 'Full Bell Lines';
		$fullAtt = 'FBL';
		break;
	}
...
<title><?php echo strip_tags(str_replace('_', ' ', $_GET["coinType"]));?> <?php echo $fullType; ?> Report</title>
...
<h1><img src="../img/<?php echo $categoryLink ?>.jpg" width="50" height="50" align="absmiddle" /> <?php echo strip_tags(str_replace('_', ' ', $_GET["coinType"]));?> <?php echo $fullType; ?> Report</h1>
<?php include("../inc/pageElements/viewTypeLinks.php"); ?>
<hr />

<table width="100%" border="0">
  <tr class="darker" align="center">
    <td width="25%">Total Attributed</td>
    <td width="25">Slabbed and Attributed</td>
    <td width="25%">&nbsp;</td>
    <td width="25%">&nbsp;</td>
  </tr>
  <tr align="center">
    <td><?php echo $collection->getTypeFullBandCount($userID, $coinType, $fullAtt); ?></td>
    <td><?php echo $collection->getTypeFullBandProCount($userID, $coinType, $fullAtt); ?></td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  </tr>
</table>
<hr />
<h3><?php echo $fullType; ?> By Year Count</h3>
...
echo '<td width="10%"><a class="brownLink" href="viewCoinYear.php?coinType='.str_replace(' ', '_', $coinType).'&year='.intval($row['coinYear']).'">'.intval($row['coinYear']).'</a> ('.$collection->getAttributeCountByYear(intval($row['coinYear']), $coinType,$userID).')</td>'; 
$i = $i + 1; //add 1 to $i
...
if(mysql_num_rows($countAll) == 0){
    ...
    <tr>
	<td>&nbsp;</td>
    <td>No '. $coinType.'  '. $fullType.' collected</td>
	<td></td><td></td>
	<td></td>	   
  </tr> ';
} else {
