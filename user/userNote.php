...
<h3><span class="glyphicon glyphicon-pencil"></span> <?php echo $Note->getNoteTitle(); ?></h3>
...
if(mysql_num_rows($sql) == 0){
	...
	echo '<option value="" selected="selected">Load Another Note</option>';
  ...
  <div class="input-group-btn">
    <a class="btn btn-default btn-danger confirmLnk" role="button" href="userNoteBOOTS.php?delete=<?php echo $_GET['ID'] ?>"> Delete Note <span class="glyphicon glyphicon-trash"></span></a>
  </div>
</div>
<hr>

<form class="form-horizontal" role="form" method="post" action="" id="noteForm">

  <div class="form-group">
  <label for="noteTitle" class="col-sm-2 control-label">Title</label>
    <div class="col-sm-10">
      <input type="text" value="<?php echo $Note->getNoteTitle(); ?>" class="form-control" id="noteTitle" name="noteTitle" placeholder="Note Title:" required>
    </div>
  </div>  
  ...
  <input name="ID" type="hidden" value="<?php echo $_GET['ID'] ?>" />
      <button type="submit" name="saveNoteBtn" class="btn btn-primary save"> Save <span class="glyphicon glyphicon-floppy-disk"></span></button>
