...
<title><?php echo strip_tags(str_replace('_', ' ', $_GET["coinType"]));?> Proof Like Report</title>
...
<h1><img src="../img/<?php echo $categoryLink ?>.jpg" width="50" height="50" align="absmiddle" /> <?php echo strip_tags(str_replace('_', ' ', $_GET["coinType"]));?> Proof Like Report</h1>
...
<?php
$countAll = mysql_query("SELECT * FROM collection WHERE coinType = '".strip_tags(str_replace('_', ' ', $_GET["coinType"]))."' AND userID = '$userID' AND morganDesignation != 'None' ORDER BY coinYear ASC") or die(mysql_error());
if(mysql_num_rows($countAll) == 0){
  ...
  echo '
	...
	<td>'. $collection->getCoinGrade().$collection->getCoinAttribute(intval($row['collectionID']), $userID).$grader.''.$collection->getGradeAttributeForCoin(intval($row['collectionID'])).'</td>
