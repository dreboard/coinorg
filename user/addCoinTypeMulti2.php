...
$.fn.changeSelect = function(i) { 
  if($(this).prop('checked')){
	  ...
	  $("#addCoinFormBtn"+i).prop('disabled', false );
  } else {
	  $("#coinquantity"+i).prop('disabled', true);
	  $("#gradeList"+i).prop('disabled', true );
	  $("#addCoinFormBtn"+i).prop('disabled', true );
  }
}	
...
<img src="../img/<?php echo $rawCoinType ?>.jpg" class="hidden-xs" align="left" id="addtypeImg" />
...
if(isset($_SESSION['errorMsg'])) {
 ...
 ?></td>
  ...
  $i = 0;
	...
	<input onclick="$(this).changeSelect(' . $coinID . ')" type="checkbox" name="coinID['.$i.'][theID]" id="coinID' . $coinID . '" value="' . $coinID . '" class="idCheck" />
      </td>
	  <td width="6%" align="center">
	  <select name="coinID['.$i.'][quantity]" class="coinsQuantity" id="coinquantity' . $coinID . '">
	...
	<td><select class="gradeLists" name="coinID['.$i.'][coinGrade]" id="gradeList' . $coinID . '">
<option value="No Grade" selected="selected">No Grade</option>
'.$collection->getGradeList($row['strike']).'
</select></td>
<td width="92%"><label  id="coinLabel' . $coinID . '" for="coinID' . $coinID . '">' . $name . '</label></td>
  </tr>';
	}
?>
   <tr class="darker">
