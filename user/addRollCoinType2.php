...
function getCoinsList($coinType, $i) {
	$coin = new Coin();

	$sql = mysql_query("SELECT * FROM coins WHERE coinType = '$coinType' AND coinYear <= ".date('Y')." AND byMint = '1'  ORDER BY coinYear ASC") or die(mysql_error()); 
	$temp_result = $sql;
	$coinCount = mysql_num_rows($sql);
	$html = '( '.$coinCount.' ) ';
	$html = '<tr class="coinsRow"><td width="2%" align="center">';
	$html .= '<select name="coinID['.$i.'][theID]" onchange="getCoin(this, '.$i.');" class="coinNumbers" id="theCoin'.$i.'"><option value="" class="coinList" selected="selected">Select A Coin</option>';
	while ($row = mysql_fetch_array($sql)){
	    ...
	    $html .= '</select>&nbsp;<span class="waitSpan" id="wait'.$i.'">Generating Grades.....</span></td>';
		$html.= '<td><select name="coinID['.$i.'][coinGrade]" id="coinGrade'.$i.'">';
...
function rollCoinCountForType($coinType, $userID){
...
mysql_query("INSERT INTO collectrolls(rollNickname, rollType, coinType, coinCategory, rollHolder, purchaseDate, purchaseFrom, auctionNumber, additional, purchasePrice, ebaySellerID, shopName, shopUrl, coinNote, coinGrade, enterDate, userID, faceVal, denomination, coincollectID, containerID) VALUES ('$rollNickname', 'Same Type', '".$coinType."', '".$CoinTypes->getCoinCategory()."', '$rollHolder', '$purchaseDate', '$purchaseFrom', '$auctionNumber', '$additional', '$purchasePrice', '$ebaySellerID', '$shopName', '$shopUrl', '$coinNote', '$coinGrade', '$theDate', '$userID', '".$CoinTypes->getRollVal()."', '".$CoinTypes->getDenomination().", '".intval($_POST["coincollectID"])."', '".intval($_POST["containerID"])."')") or die(mysql_error()); 
...
break;	
case '1';
// CREATE add bulk coins function
$sql = mysql_query("INSERT INTO collectionBulk(coinID, collectrollsID, coins, coinType, coinCategory, enterDate, userID, denomination) VALUES('0', '$collectrollsID', '".$coin->getMintMark()."', ".$CoinTypes->getRollCount().", '$coinType', '".$CoinTypes->getCoinCategory()."', '$theDate', '$userID'')") or die(mysql_error()); 
break;		
}
...
$(".detailDiv2, .waitSpan, #addCoinsList, #addCoinsList2, #addNoneDiv").hide();
...
$("#addCentForm").submit(function() {
      ...
      return false;
      }else if($("#addCentForm input:radio:checked").length < 1) {
		$("#errorMsg").text("Please Make Entry Selection");
		$(".addRadiosLbs").addClass("errorTxt");
		$("#endErrorMsg").text("There are form errors... Scroll up to fix");
		return false;
	}else {
...
$("#rollNickname, .addRadios").click(function(){
...
$("#addCoins").click(function(){
  if($(this).prop('checked',false)){
	  $(".coinNumbers").prop('selectedIndex',0);
  }
});	
...
<table width="100%" border="0">
  ...
  <tr>
    <td><strong>Roll Type:</strong> Same Type</td>
  </tr>
</table>
...
<table width="100%" border="0" id="addingCoinsLblTbl">
  <tr>
    <td width="123" class="darker">Individually</td>
    <td width="30"><input type="radio" name="addCoins" id="addCoins" class="addRadios" value="0" /></td>
    <td width="833" class="addingCoinsTblLbl"><label for="addCoins" class="addRadiosLbs">I Will Choose Each Coin</label></td>
    </tr>
  <tr>
    <td class="darker">Auto Insert</td>
    <td><input type="radio" name="addCoins" id="addAll" value="1" class="addRadios" /></td>
    <td><label for="addAll" class="addRadiosLbs">Add All (<strong>*</strong>Select for sealed rolls)</label></td>
    </tr>
  <tr>
    <td class="darker">Dont Add Any</td>
    <td><input type="radio" name="addCoins" id="addNone" value="2" class="addRadios" /></td>
    <td><label for="addNone" class="addRadiosLbs"> Will assign coins from my collection/label></label></td>
  </tr>
</table>
    
    <br />
<div id="addCoinsList2">
<p class="brownLink">All <span class="darker"><?php echo $CoinTypes->getRollCount() ?> <?php echo str_replace('_',' ',  $_GET["coinType"]); ?></span> coins will be added to your bulk coins list<br />
*Bulk coins <strong>can not be viewed individually</strong>, but will be included in your overall count.</p>
</div>

<div id="addNoneDiv">

<strong><?php echo rollCoinCountForType($_GET["coinType"], $userID) ?></strong> Available Coin(s) In Collection.  Coins can be added after roll is created.
</div>


<div id="addCoinsList">    
  <p class="brownLink">Add each (up to <span class="darker"><?php echo $CoinTypes->getRollCount() ?> <?php echo str_replace('_',' ',  $_GET["coinType"]); ?></span> coins) to this roll</p>
<table width="100%" border="0">
  <tr class="darker">
    <td>Coin</td>
...
for ($i=1; $i<=$CoinTypes->getRollCount(); $i++) {
	//echo getCoinsList($coinType, $i);
	
	$sql = mysql_query("SELECT * FROM coins WHERE coinType = '".str_replace('_',' ',  $_GET["coinType"])."' AND coinYear <= ".date('Y')." AND byMint = '1'  ORDER BY coinYear ASC") or die(mysql_error()); 
	$coinCount = mysql_num_rows($sql);
...
if(isset($_POST["coincollectID"])){echo '<option value="'.$_POST["coincollectID"].'" selected="selected">'.$_POST["coincollectID"].'</option>';
  ...
  <table width="979" border="0" class="addCoinTbl">  
  <tr>
    <td width="186"><label for="file">Image</label></td>
    <td width="783">
