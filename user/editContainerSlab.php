...
mysql_query("UPDATE containers SET containerName = '$containerName', purchaseDate = '$purchaseDate', purchaseFrom = '$purchaseFrom', shopName = '$shopName', shopUrl = '$shopUrl', showName = '$showName', showCity = '$showCity', auctionNumber = '$auctionNumber', ebaySellerID = '$ebaySellerID', coinNote = '$coinNote' WHERE containerID = '$containerID' AND userID = '$userID' ") or die(mysql_error()); 

header("location: ".$Container->getContainerTypeURLByID($containerID)." ");
...
<form action="" method="post" enctype="multipart/form-data" name="addCollectionForm" id="addCollectionForm">
  ...
  <tr>
    ...
    <td colspan="2"><select id="purchaseFrom" name="purchaseFrom">
      <option value="None" selected="selected"><?php echo $Container->getPurchaseFrom(); ?></option>      
  ...
  <tr>
    ...
    <table width="979" border="0" class="addCoinTbl" cellpadding="3">
  <tr>
    <td width="189" valign="top"><label for="coinNote">Description</label></td>
    <td width="772" colspan="4"><textarea name="coinNote" id="coinNote" class="wideTxtarea" cols="" rows="6"><?php echo $Container->getCoinNote(); ?></textarea></td>
  </tr>
  <tr>
    <td class="darker">
