...
break;	
case '1';
$collection->autoInsertCoinTypeIntoRoll($coinType, $collectrollsID, $userID);
break;	
case '2';
//Make 50 new
if (isset($_POST["noBulk"])) { 
  //$collection->insertRollToBulk($userID, $collectrollsID, $coinType);
} else {
	//$collection->autoInsertNewCoinTypeIntoRoll($coinType, $collectrollsID, $userID);
}
...
$(".detailDiv2, .waitSpan, .addChoices").hide();
...
<table width="100%" border="0">
  ...
  <tr>
    <td><strong>Roll Type:</strong> <a href="addRollSameType.php">Same Type</a></td>
  </tr>
</table>
...
<form role="form" action="" method="post" enctype="multipart/form-data" name="addCentForm" id="addCentForm">
...
<table width="100%" border="0" id="addingCoinsLblTbl">
      <tr>
    <td class="darker">All New</td>
    <td><input type="radio" name="addCoins" id="addNew" value="2" class="addRadios" /></td>
    <td><label for="addNew" class="addRadiosLbs">Add All New (ie: Select for sealed rolls)</label></td>
    </tr>  
    <tr>
    <td width="123" class="darker">Individually</td>
    <td width="30"><input type="radio" name="addCoins" id="addEach" class="addRadios" value="0" /></td>
    <td width="833" class="addingCoinsTblLbl"><label for="addCoins" class="addRadiosLbs">I Will Choose Each New Coin</label></td>
    </tr>
  <tr>
    <td class="darker">Auto Insert</td>
    <td><input type="radio" name="addCoins" id="addAll" value="1" class="addRadios" /></td>
    <td><label for="addAll" class="addRadiosLbs">Add All Available From My Collection</label></td>
    </tr>

  <tr>
    <td class="darker">Dont Add Any</td>
    <td><input name="addCoins" type="radio" class="addRadios" id="addNone" value="3" checked="checked" /></td>
    <td><label for="addNone" class="addRadiosLbs"> I will assign coins from my collection <strong>(Default)</strong></label></td>
  </tr>
</table>
    
    <br />
<div id="addCoinsList2" class="addChoices">
<p>All <span class="darker">
<?php echo $collection->gexMaxTypeRollNumberDisplay (str_replace('_',' ',  $_GET["coinType"]), $userID); ?> of <?php echo $CoinTypes->getRollCount() ?> <?php echo str_replace('_',' ',  $_GET["coinType"]); ?></span> coins will be added to this roll.</p>
</div>

<div id="addNoneDiv" class="addChoices">
<strong><?php echo $collection->availableTypeCoinsRequest($_GET["coinType"], $userID) ?></strong> Available Coin(s) In Collection.  Coins can be added after roll is created.
</div>

<div id="addNewDiv" class="addChoices">
<strong>50 New <?php echo str_replace('_',' ',  $_GET["coinType"]); ?> Coins</strong> Will Be Added To Your Bulk Collection.
<table width="100%" border="0">
  <tr>
    <td width="8%" align="center"><input type="checkbox" name="noBulk" id="noBulk" />
      <label for="noBulk"></label></td>
    <td width="92%">Add Individual Coins To My Collection <strong>(Not Bulk)</strong></td>
  </tr>
</table>

</div>

<div id="addCoinsList" class="addChoices">    
...
if(isset($_POST["coincollectID"])){echo '<option value="'.$_POST["coincollectID"].'" selected="selected">'.$_POST["coincollectID"].'</option>';
  ...
  <tr>
    ...
    <input class="btn btn-primary" name="addCoinFormBtn" id="addCoinFormBtn" type="submit" value="Add Mint Roll" />
</td>
