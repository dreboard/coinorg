...
function insertRollCoinID($collectrollsID, $coinID, $theDate, $userID, $coinGrade){
	$coin = new Coin();
	...
	$sql = mysql_query("INSERT INTO collection(coinID, collectrollsID, mintMark, coinType, coinCategory, coinSubCategory, coinYear, coinVersion, strike, slabLabel, coinGrade, coinGradeNum, purchaseDate, purchaseFrom, auctionNumber, additional, purchasePrice, ebaySellerID, shopName, shopUrl, coinNote, enterDate, userID, errorCoin, byMint, mintBox, century, design, series, designType, showName, showCity, commemorativeType, commemorativeVersion, commemorative, denomination, keyDate, coinMetal) VALUES('$coinID', '$collectrollsID', '".$coin->getMintMark()."', '".$coin->getCoinType()."', '".$coin->getCoinCategory()."', 'None', '".$coin->getCoinYear()."', '".$coin->getCoinVersion()."', '".$coin->getCoinStrike()."', '$slabLabel', '$coinGrade', '".$collection->getCoinGradeNum($coinGrade)."', '$purchaseDate', '$purchaseFrom', '$auctionNumber', '$additional', '$purchasePrice', '$ebaySellerID', '$shopName', '$shopUrl', '$coinNote', '$theDate', '$userID', '".$coin->getErrorCoin()."', '".$coin->getByMint()."', 'No', '".$coin->getCentury()."', '".$coin->getDesign()."', '".$coin->getSeries()."', '".$coin->getDesignType()."', '$showName', '$showCity', '".$coin->getCommemorativeType()."', '".$coin->getCommemorativeVersion()."', '".$coin->getCommemorative()."', '".$coin->getDenomination()."', '".$coin->getKeyDate()."', '".$coin->getCoinMetal()."')") or die(mysql_error()); 
  ...
  foreach ($_POST["coinID"] as $coinID){
	if($coinID['theID'] !=='0'){
	   echo $coinID["theID"].' '. $coinID["coinGrade"].'<br />';
		insertRollCoinID($collectrollsID, $coinID=$coinID["theID"], $theDate, $userID, $coinGrade=$coinID["coinGrade"]);
	}
  }
}
//////////add file
...
if(mysql_num_rows($collectionQuery) == 0){
	  echo '
		  <tr>
		  <td colspan="3">You dont have any same year coin rolls In Your Collection</td> 
		  </tr>  ';
} else {
...
while($row = mysql_fetch_array($collectionQuery))
  ...
  echo '
<tr>
<td><a href="viewRollDetail.php?ID='.$Encryption->encode($collectrollsID).'">'.$CollectMintRolls->getRollName().'</a></td>
