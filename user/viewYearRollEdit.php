...
$availableCoinsRequest = mysql_query("SELECT * FROM collection WHERE coinYear = '".$collectionRolls->getCoinYear()."' AND coinType = '$coinType' AND proservice = 'None' AND collectfolderID = '0' AND collectrollsID = '0' AND collectsetID = '0' AND setregID = '0' AND userID = '$userID'");
...
header("location: viewYearRollEdit.php?ID=".$_POST["ID"]."");
exit();
}
//All
...
header("location: viewYearRollEdit.php?ID=".$Encryption->encode($collectrollsID)."");
exit();
}
...
<tr align="center" class="darker">
    <td width="25"><h3><a href="viewYearRollDetail.php?ID=<?php echo $Encryption->encode($collectrollsID) ?>">Roll Main View</a></h3></td>
    <td width="25%"><h3><a href="editRollSameYearForm.php?ID=<?php echo $Encryption->encode($collectrollsID) ?>">Edit This Roll Details</a></h3></td>
    <td width="25%"><h3><a href="addRollSameMintDiffYrs.php">Add Same Year Roll</a></h3></td>
...
<table width="100%" border="0">
  <tr>
     <td width="50%">
     <img src="../img/Lincoln_Memorialplaceholder.jpg" width="30" height="30" align="absmiddle" />  <strong><a href="viewYearRollNewCoins.php?ID=<?php echo $Encryption->encode($collectrollsID); ?>">Add New Coins To Roll: </a></strong>
      <br />
<span id="enterOpenCoinsSpan"> *Add New Coins To Collection</span>
    ...
    <input name="removeAllCoinsBtn" id="removeAllCoinsBtn" type="submit" value="Remove All" onclick="if (confirm('Remove All Coins?')) { document.getElementById('removeAllCoinsBtn').setAttribute('value', 'Deleting...');}" />
    <br />
<span id="removeAllCoinsSpan">*Keep Roll</span>
  ...
  <tr>
    ...
    <form action="" method="post" class="compactForm" id="deleteFolderForm">
        ...
        <input name="ID" type="hidden" value="<?php echo $Encryption->encode($collectrollsID); ?>" />
    <input name="deleteRollBtn" id="deleteRollBtn" type="submit" value="Remove Roll" onclick="if (confirm('Delete Roll And Keep Coins?')) { document.getElementById('deleteRollBtn').setAttribute('value', 'Deleting...');}" />
    <br />
...
<p class="innerAlbum">Use the forms to search and add/update the roll coins.  The list will only be populated by the coins in your collection.  If you have a coin that meets this rolls criteria, you can go to the <a href="viewYearRollNewCoins.php?ID=<?php echo $Encryption->encode($collectrollsID); ?>" class="brownLinkBold">Add New Coins To Roll: </a> page and enter the collection data.</p>
<h2>Current Coins</h2>
<?php 
...
<label for="collectionID"><strong><?php echo mysql_num_rows($availableCoinsRequest) ?></strong> Available Coins In Collection </label>
...
<?php 
$sql  = mysql_query("SELECT * FROM collection WHERE coinYear = '".$collectionRolls->getCoinYear()."' AND coinType = '$coinType' AND proservice = 'None' AND collectfolderID = '0' AND collectrollsID = '0' AND collectsetID = '0' AND setregID = '0' AND userID = '$userID'");
...
?>
        ...
        <input name="ID" type="hidden" value="<?php echo $Encryption->encode($collectrollsID); ?>" />
    <input name="addRollCoinsBtn" id="addRollCoinsBtn" type="submit" value="Add Coin" onclick="if (confirm('Add Coin to Roll?')) { document.getElementById('addRollCoinsBtn').setAttribute('value', 'Adding...');}" /> 
...
<?php
$i = 1;
$result  = mysql_query("SELECT * FROM collection WHERE coinYear = '".$collectionRolls->getCoinYear()."' AND coinType = '$coinType' AND proservice = 'None' AND collectfolderID = '0' AND collectrollsID = '0' AND collectsetID = '0' AND setregID = '0' AND userID = '$userID'");
...
<table width="98%" border="0" id="coinChangerTbl" cellpadding="3">
  <tr class="darker">
    <td width="46%">Coin</td>
    <td width="49%">Other Coins In Collection</td>
    <td width="5%">&nbsp;</td>
  </tr>
<?php 
$i = 0;
	$sql  = mysql_query("SELECT * FROM collection WHERE collectrollsID = '".$collectrollsID."' AND userID = '$userID' LIMIT ".$CoinTypes->getRollCount()."") or die(mysql_error()); 
	$rollCount = mysql_num_rows($sql); 
	while($row = mysql_fetch_array($sql)){
		$i++;
        $coin->getCoinById(intval($row['coinID']));
		$collection->getCollectionById(intval($row['collectionID']));
	echo '<tr>
    <td><a href="viewCoinDetail.php?ID='.$Encryption->encode(intval($row['collectionID'])).'">' .$General->summary($coin->getCoinName(), $limit=60, $strip = false). '</a> ' .$collection->getCoinGrade() . '</td>
    <td>'.$collectionRolls->otherYearCoinsList(intval($row['collectionID']), $coinType, $userID, $collectrollsID).'</td>
	...
	<input name="noRollBtn" class="noRollBtns" id="changeBtn'.$i.'" type="submit" value="X" onclick="return confirm(\'Remove This Coin From Roll?\')" /></form></td>
  </tr>';
}
?>
</table>
