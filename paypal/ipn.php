<?php
require_once"../inc/configSite.php";
...
if ($receiver_email != "admin@mycoinorganizer.com") {
    ...
    mail("admin@mycoinorganizer.com", "Receiver Email is incorrect", $message, "From: store@mycoinorganizer.com" );
    exit(); // exit script
}
// Check number 2 ------------------------------------------------------------------------------------------------------------
...
if ($numRows > 0) {
    ...
    mail("admin@mycoinorganizer.com", "Duplicate txn_id in the IPN system", $message, "From: store@mycoinorganizer.com" );
    exit(); // exit script
} 
// Check number 4 ------------------------------------------------------------------------------------------------------------
...
if ($fullAmount != $grossAmount) {
        ...
        mail("admin@mycoinorganizer.com", "Price Jack or Bad Programming", $message, "From: store@mycoinorganizer.com" );
        exit(); // exit script
} 
...
$sql = mysql_query("INSERT INTO subscription (payer_email, payment_date)  VALUES('$payer_email', '$payment_date')") or die ("unable to execute the query");
...
mail("admin@mycoinorganizer.com", "Store IPN RESULT YAY MONEY!", $req, "store@mycoinorganizer.com");
?>