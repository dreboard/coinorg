...
<title>Small Cent Coins Sets</title>
...
if (isset($_SESSION["cart_array"]) || count($_SESSION["cart_array"]) >= 1) {
 ...
 $IDS = mysql_real_escape_string(implode(',', $keys));
		$sql = mysql_query("SELECT * FROM products WHERE  (NOT FIND_IN_SET(productID, '$IDS')) AND siteID = '1' AND quantity >= '1' AND category = 'Sets' ORDER BY product_name ASC");
...
echo '<td width="25%" height="150" align="center">
 ...
 <strong>$'.$Product->getPrice().'</strong>
	...
	$sql = mysql_query("SELECT * FROM products WHERE category = 'Sets' AND siteID = '1' AND quantity >= '1' ORDER BY product_name ASC");
    if(mysql_num_rows($sql) == 0){
	  echo '
	    <tr>
		  <td width="25%">No sets In inventory</td>
