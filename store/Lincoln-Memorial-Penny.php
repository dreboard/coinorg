...
<meta name="keywords" content="Lincoln Memorial Coins, Lincoln penny, 1982 7 Coin Variety Set, copper coins, 1995 DDO, double die pennies">
<title>Lincoln Memorial Penny Coins, Sets &amp; Rolls</title>
...
<h1>Lincoln Memorial Penny Coins, Sets &amp; Rolls</h1>
...
<hr /> 
  <table width="100%" border="0" id="productTbl">
    <thead class="darker">
  <tr align="center">
    <td width="25%" align="left"><a title="Lincoln Memorial Price List" href="product_list.php?coinType=Lincoln_Memorial" class="brownLinkBold">List View</a></td>
    <td width="25%">&nbsp;</td>
    <td width="25%">&nbsp;</td>
    <td width="25%">&nbsp;</td>
  </tr>
...
if (isset($_SESSION["cart_array"]) || count($_SESSION["cart_array"]) >= 1) {
 ...
 $IDS = mysql_real_escape_string(implode(',', $keys));
		$sql = mysql_query("SELECT * FROM products WHERE  (NOT FIND_IN_SET(productID, '$IDS')) AND siteID = '1' AND quantity >= '1' AND coinType = 'Lincoln Memorial' ORDER BY product_name ASC");
...
<input type="image" src="img/addCartBtn.jpg" name="addCartBtn" id="addCartBtn" onclick="addCart();" />
	...
	$sql = mysql_query("SELECT * FROM products WHERE coinType = 'Lincoln Memorial' AND quantity >= '1' AND siteID = '1' ORDER BY product_name ASC");
    if(mysql_num_rows($sql) == 0){
	  echo '
    <td width="25%">No Lincoln Memorial coins In inventory</td>
    <td width="25%">&nbsp;</td>
